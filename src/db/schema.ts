 ...
 * - Artifacts are versioned slots per (owner, agent[, role later]); the
 *   `status` field is pre-wired for the draft-&-approve step.
 */
...
export const artifacts = pgTable(
  "artifacts",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    agentSlug: text("agent_slug").notNull(),
    // Slot key = (createdBy, agentSlug[, roleId when roles land]). Version
    // increments within the slot; history is kept.
    ...
    // Reserved for build-queue step 3 (roles/requisitions):
    roleId: uuid("role_id"),
    createdBy: text("created_by")
      .notNull()
      .references(() => users.id),
  ...
  (t) => [index("artifacts_slot_idx").on(t.createdBy, t.agentSlug, t.version)]
);
