...
function JobDescriptionAgent() {
  ...
  function switchAgent(next: AgentSlug) {
    ...
    router.replace(`/agents?${qp.toString()}`);
  }
  ...
  function selectProject(id: string, title?: string) {
    ...
    setCoverage(null);
    setError(null);
    router.replace(`/agents?${new URLSearchParams({ agent, project: id }).toString()}`);
  }
  ...
  useEffect(() => {
    ...
    fetch("/api/projects")
      ...
      .then((d) => {
        // /api/projects splits owned vs shared since ADR-008; this legacy
        // picker just needs everything the user can write to.
        if (alive) setProjects([...(d.owned ?? []), ...(d.shared ?? [])]);
      })
      .catch(() => {});
    return () => {
      alive = false;
