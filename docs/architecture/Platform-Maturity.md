...
| Evaluation | 🟢 LIVE (2026-07-19, pulled forward) | `scripts/run-evals.ts` (tsx) runs 10 golden cases from `products/interview-intelligence/evals/` against the real runtime: fictional fixture family (Head of DevOps), structural checks + strict LLM judge (0–10 rubric) + guardrail probes (off-scope, prompt-extraction). Results → `generated/evals/run-*/`. First run caught the max_tokens/thinking empty-reply bug and the JD 8192 truncation. Not yet: CI wiring, judge sees fixtures (verifies name-fidelity claims), multi-case-per-agent depth |
...
- **Prompt version:** v1.1 (2026-07-19) — added mandatory document-ingest behavior: an uploaded/pasted JD or brief is swept against the full questionnaire, extracted answers credited as `source=document`, interview continues from the gaps only (verified by the multi-turn eval case). System prompt assembled at runtime by `src/orchestrator/job-description-orchestrator.ts`; the same `buildJobDescriptionSystemPrompt()` serves text chat (`/api/job-description`) and the voice custom-LLM callback (`/api/job-description/voice-llm`) — one brain. Model: `claude-sonnet-5`, `max_tokens: 16384` (2048 → 8192 → 16384 on 2026-07-19: adaptive thinking + the full JD + 20-section coverage JSON truncated at lower budgets — found by evals). Generic-registry agents default to 8192 with an empty-reply retry guard.
